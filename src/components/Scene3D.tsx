...
function WaypointDots({
  ...
  // One shared geometry for all discs
...
function SceneContent({
  ...
  // Computed inline — cheap lerp, fine to run every render
  const {
    currentLat,
    currentLng,
    currentWidth,
  ...
  // Memoized on currentSegIdx (discrete integer) — not every frame
  ...
  // ── Path line geometry — memoized on segment index (not every frame) ───────
...
export default function Scene3D({ townModel, agentOutputs }: Scene3DProps) {
  ...
  // pathSegments derived here so TimeSlider (outside Canvas) can access them
  const pathSegments = useMemo<PathSegment[]>(
    () => agentOutputs['path']?.data.path_segments ?? [],
    [agentOutputs],
  );

  // Stable callback: update progress and pause (used for scrubbing)
